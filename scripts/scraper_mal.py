...
DEFAULT_OUTPUT_PATH = PROJECT_ROOT / "data" / "raw" / "anime_sentiment.csv"
DEFAULT_BACKUP_PATH = PROJECT_ROOT / "data" / "raw" / "resenas_backup.csv"
...
def extraer_resenas_anime(
    ...
    for pagina in range(1, paginas_por_anime + 1):
        ...
        for caja in cajas:
            ...
            if recomendacion is None or texto is None:
                continue
            ...
            filas.append(
                {
                    ...
                    "sentimiento": 1 if recomendacion == "recommended" else 0,
                }
            )
            nuevos += 1

        print(f"  pagina {pagina}: {nuevos} resenas validas")
        time.sleep(random.uniform(min_delay, max_delay))
...
def parsear_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scraper de resenas de MyAnimeList")
    parser.add_argument("--max-anime", type=int, default=500)
    parser.add_argument("--paginas-por-anime", type=int, default=30)
...
def main() -> int:
    args = parsear_args()
    print("Iniciando extraccion de datos de MyAnimeList...")
    ...
    if not filas_totales:
        print("No se extrajeron resenas validas. Revisa conectividad o selectores.")
        return 1

    guardar_csv(args.salida, filas_totales)
    print(f"Extraccion completa: {len(filas_totales)} resenas en {args.salida}")
    return 0
